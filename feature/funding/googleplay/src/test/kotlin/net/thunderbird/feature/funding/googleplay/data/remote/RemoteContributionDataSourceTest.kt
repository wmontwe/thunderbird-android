...
class RemoteContributionDataSourceTest {
    ...
    fun `getAllOneTime should return outcome from billingClient`() = runTest {
        ...
        billingClient.oneTimeFlow = flowOf(expectedOutcome)
    ...
    fun `getAllRecurring should return outcome from billingClient`() = runTest {
        ...
        val expectedOutcome = Outcome.success(contributions)
        billingClient.connectOutcome = Outcome.success(Unit)
        billingClient.recurringFlow = flowOf(expectedOutcome)
    ...
    fun `getAllPurchased should combine one-time and recurring purchases`() = runTest {
        ...
        billingClient.purchasedOneTimeFlow = flowOf(Outcome.success(listOf(oneTime)))
        billingClient.purchasedRecurringFlow = flowOf(Outcome.success(listOf(recurring)))
        ...
        assertThat(data).isEqualTo(listOf(oneTime, recurring))
    }
    ...
    @Test
    fun `getAllPurchased should return failure when one-time fetch fails`() = runTest {
        val error = ContributionError.UnknownError("Failed")
        billingClient.purchasedOneTimeFlow = flowOf(Outcome.failure(error))
        billingClient.purchasedRecurringFlow = flowOf(Outcome.success(emptyList()))
